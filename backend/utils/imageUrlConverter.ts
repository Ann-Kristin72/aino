...
export class ImageUrlConverter {
  private static readonly AZURE_BASE_URL = 'https://ainomedia.blob.core.windows.net/aino-media';
  
  /**
   * Convert local image URLs in HTML content to Azure Blob Storage URLs
   */
  static convertHtmlContent(htmlContent: string): string {
  ...
  static convertImageUrl(imageUrl: string): string {
    ...
    if (imageUrl.includes('skillaidmedia.blob.core.windows.net')) {
      ...
      const imagePath = urlParts.slice(-2).join('/'); // Get the last two parts (media/image/filename)
   ...
   */
  static convertMarkdownImages(markdownContent: string): string {
  ...
  static extractImageUrls(htmlContent: string): string[] {
    ...
    const bgMatches = htmlContent.match(/background-image:\s*url\(['"]?([^'")\s]+)['"]?\)/gi);
    if (bgMatches) {
