...
router.get("/", async (req, res) => {
    try {
        ...
        const mediaItems = await db_1.db.select().from(schema_1.media).orderBy(schema_1.media.createdAt);
...
router.post("/upload", upload.single("file"), async (req, res) => {
    try {
        ...
        // Lagre i database
        const [newMedia] = await db_1.db.insert(schema_1.media).values({
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        const mediaItem = await db_1.db.select().from(schema_1.media).where((0, drizzle_orm_1.eq)(schema_1.media.id, parseInt(id))).limit(1);
